...
export default () => {
  ...
  const handleSubmit = (e) => {
    ...
    emailjs
      .sendForm('service_74s1osh', 'template_26d4qbw', form, 'XjfTUrxNFciFEHY5Z')
      .then((response) => {
  ...
  return (
    <main className="py-20 bg-[#F7F6BB]">
              ...
              {otherDetails.map((detail, idx) => (
                <div key={idx}>
                  ...
                  <input
                    ...
                    style={{ backgroundColor: '#FFF', outlineColor: '#C1F2B0' }}
                  />
                </div>
              ))}
              <div>
                ...
                <textarea required value={message} onChange={(e) => setMessage(e.target.value)} className="w-full mt-2 h-36 px-3 py-2 resize-none appearance-none bg-transparent outline-none border focus:border-indigo-600 shadow-sm rounded-lg" style={{ backgroundColor: '#FFF', outlineColor: '#C1F2B0' }}></textarea>
              </div>
              <button
                type="submit"
                className="w-full  bg-[#2C7865]  px-4 py-[10px] font-semibold text-white hover:text-black hover:bg-gradient-to-tl hover:from-[#C1F2B0] hover:to-[#65B741]"
              >
                Submit
              </button>
            </form>
